...
CLUSTER_NAMES_MAP = {2: ["Chill Vibes 🌙",
                         "High Energy 🔥"],
                     3: ["Chill Vibes 🌙",
                         "Balanced Mix 🎼",
                         "High Energy 🔥"],
...
def load_data():
    # Only load the columns we actually need to save memory on Streamlit Cloud
    # (1GB RAM limit)
    usecols = [
        "track_name",
        "artists",
    ...
    df["loudness_norm"] = ((df["loudness"] - l_min) /
                           (l_max - l_min)).astype("float32")

    t_min, t_max = df["tempo"].min(), df["tempo"].max()
    df["tempo_norm"] = ((df["tempo"] - t_min) /
                        (t_max - t_min)).astype("float32")
...
with st.sidebar:
    ...
    st.markdown(
        '<p class="sidebar-label">🔬 Clustering</p>',
        unsafe_allow_html=True)
    ...
    st.markdown(
        '<p class="sidebar-label">🎛️ Filters</p>',
        unsafe_allow_html=True)
    ...
    st.markdown(
        '<p class="sidebar-label">🎨 Visualization</p>',
        unsafe_allow_html=True)
...
with st.spinner("🎵 Loading & clustering your Spotify universe..."):
    ...
    if selected_genres:
        df_filtered = df_filtered[df_filtered["track_genre"].isin(
            selected_genres)]
    ...
    clustered, sil_score, cluster_centers, pca_variance = run_clustering(
        n_clusters, sample_size
    )

    # Merge cluster labels back where possible
    cluster_names = CLUSTER_NAMES_MAP.get(
        n_clusters, [f"Cluster {i}" for i in range(n_clusters)]
    )
...
metrics = [
    ...
    },
    {
        ...
        "delta": "K-Means Algorithm",
        "delta_class": "delta-purple",
    ...
    {
        "icon": "⚡",
        "value": f"{df_raw['energy'].mean():.2f}",
        "label": "Avg Energy",
        "delta": f"Max: {df_raw['energy'].max():.2f}",
        "delta_class": "delta-orange",
        "gradient": "var(--gradient-orange)",
    },
    {
        "icon": "💃",
        "value": f"{df_raw['danceability'].mean():.2f}",
        "label": "Avg Danceability",
        "delta": f"Valence: {df_raw['valence'].mean():.2f}",
        "delta_class": "delta-blue",
...
with c1:
    st.markdown(
        '<div class="chart-card"><div class="chart-title">PCA 2D Cluster Map</div><div class="chart-sub">Principal Component Analysis — audio feature space projection</div>',
        unsafe_allow_html=True,
    )
...
with c2:
    st.markdown(
        '<div class="chart-card"><div class="chart-title">Cluster Distribution</div><div class="chart-sub">Track count per cluster segment</div>',
        unsafe_allow_html=True,
    )
...
if show_3d:
    ...
    st.markdown(
        '<div class="chart-card"><div class="chart-title">3D PCA Cluster Visualization</div><div class="chart-sub">Rotate & zoom to explore the three principal dimensions of audio DNA</div>',
        unsafe_allow_html=True,
    )
...
st.markdown(
    '<div class="chart-card"><div class="chart-title">Audio Feature Radar by Cluster</div><div class="chart-sub">Normalized 0–1 scale — each axis represents an audio attribute</div>',
    unsafe_allow_html=True,
)
fig_radar = go.Figure()
for i in range(n_clusters):
    ...
    # Normalize to 0-1
    vals_norm = [
        (v - df_raw[f].min()) / (df_raw[f].max() - df_raw[f].min() + 1e-9)
        for v, f in zip(vals, radar_features)
    ]
...
with col_heat:
    st.markdown(
        '<div class="chart-card"><div class="chart-title">Audio Feature Correlation Matrix</div><div class="chart-sub">Pearson correlation between all numeric features</div>',
        unsafe_allow_html=True,
    )
    corr_features = [
        "danceability",
...
with col_box:
    st.markdown(
        '<div class="chart-card"><div class="chart-title">Energy vs Danceability by Genre</div><div class="chart-sub">Scatter density — top 12 genres by popularity</div>',
        unsafe_allow_html=True,
    )
    top_genres = (df_raw.groupby("track_genre")[
        "popularity"].mean().nlargest(12).index.tolist())
    df_top = df_raw[df_raw["track_genre"].isin(top_genres)].sample(
        n=min(5000, len(df_raw)), random_state=42
    )
...
with col_g1:
    st.markdown(
        '<div class="chart-card"><div class="chart-title">Top Genres by Avg Popularity</div><div class="chart-sub">20 genres ranked by mean track popularity score</div>',
        unsafe_allow_html=True,
    )
    genre_pop = (df_raw.groupby("track_genre")[
        "popularity"].mean().nlargest(20).reset_index())
    genre_pop.columns = ["Genre", "Avg Popularity"]
    genre_pop = genre_pop.sort_values("Avg Popularity")
    colors_bar = [
        f"rgba(29,185,84,{0.4 + 0.6 * i / len(genre_pop)})"
        for i in range(len(genre_pop))
    ]
    fig_genre = go.Figure(
        go.Bar(
    ...
    st.plotly_chart(
        fig_genre, use_container_width=True, config={"displayModeBar": False}
    )
    st.markdown("</div>", unsafe_allow_html=True)

with col_g2:
    st.markdown(
        '<div class="chart-card"><div class="chart-title">Audio Feature Distributions</div><div class="chart-sub">Violin plots showing spread per feature</div>',
        unsafe_allow_html=True,
    )
    ...
    top5_genres = (df_raw.groupby("track_genre")[
        "popularity"].mean().nlargest(6).index.tolist())
    ...
    st.plotly_chart(
        fig_viol, use_container_width=True, config={"displayModeBar": False}
    )
    st.markdown("</div>", unsafe_allow_html=True)
...
with col_p1:
    st.markdown(
        '<div class="chart-card"><div class="chart-title">Popularity Distribution</div><div class="chart-sub">How popular are the tracks in this dataset?</div>',
        unsafe_allow_html=True,
    )
    fig_hist = go.Figure()
    fig_hist.add_trace(
...
with col_p2:
    st.markdown(
        '<div class="chart-card"><div class="chart-title">Tempo vs Energy</div><div class="chart-sub">Hexbin density map — warmer = more tracks</div>',
        unsafe_allow_html=True,
    )
...
if show_elbow:
    ...
    st.markdown(
        '<div class="chart-card"><div class="chart-title">Elbow Curve + Silhouette Scores</div><div class="chart-sub">Find optimal number of clusters — elbow = best K tradeoff</div>',
        unsafe_allow_html=True,
    )
    ...
    fig_elbow.update_yaxes(
        title_text="Inertia (WCSS)", secondary_y=False, **grid_style()
    )
    fig_elbow.update_yaxes(
        title_text="Silhouette Score", secondary_y=True, **grid_style()
    )
    st.plotly_chart(
        fig_elbow, use_container_width=True, config={"displayModeBar": False}
    )
    st.markdown("</div>", unsafe_allow_html=True)
...
st.markdown(
    '<div class="chart-card"><div class="chart-title">Audio Feature Averages per Cluster</div><div class="chart-sub">Mean values of each audio feature — hover cells for details</div>',
    unsafe_allow_html=True,
)
...
cell_vals = [profile_df.index.tolist()] + [
    profile_df[c].round(3).tolist() for c in profile_df.columns
]
cell_colors_rows = []
...
cluster_col_colors = [hex_to_rgba(
    CLUSTER_COLORS[i % len(CLUSTER_COLORS)], 0.2) for i in range(n_clusters)]
...
# Curated High-Res Unsplash Poster maps for all genres (4 unique posters
# per genre)
GENRE_POSTER_MAP = {
    "pop": [
...
df_rec_pool = df_raw[df_raw["track_genre"].str.contains(
    target_g, case=False, na=False)]
    ...
    if idx < len(rec_sample):
        ...
        with col:
            ...
            if target_g in (
                ...
                kick = np.sin(2 * np.pi * 55 * t) * \
                    kick_env * 0.5  # 808 sub kick
                bass = _sqr(root * 0.5, amp=0.3, n=6)
                lead = _saw(root * 2, amp=0.2 * t_energy, n=6)
                hat_noise = np.random.default_rng(
                    42).standard_normal(len(t)) * 0.05
                ...
                # ── Synthwave ──
                # Detuned twin sawtooth pads + slow arpeggio + retro square
                # melody
                saw1 = _saw(root, amp=0.22, n=10)
                saw2 = _saw(
                    root * 1.005, amp=0.22, n=10
                ...
                # ── Acoustic / Folk ──
                # Gentle plucked guitar chord (root + maj3rd + fifth) with soft
                # decay
                maj3 = root * 1.260
                fifth = root * 1.498
                ...
                vibrato = 1.0 + 0.005 * \
                    np.sin(2 * np.pi * 5.5 * t)  # 5.5 Hz vibrato
                root_v = root * vibrato
                ...
                # ── Indie ──
                # Jangly major 7th chord (sine + slight saw texture) + lo-fi
                # noise floor
                ...
                lo_fi = np.random.default_rng(
                    13).standard_normal(len(t)) * 0.02
                ...
                bol_env = (
                    np.abs(np.sin(np.pi / tabla_period * t)) ** 3
...
for i, tab in enumerate(tabs):
    with tab:
        top_tracks = (
            clustered[clustered["cluster"] == i]
            .nlargest(8, "popularity")[
                [
                    "track_name",
                    "artists",
                    "popularity",
        ...
        for idx, row in top_tracks.iterrows():
            ...
            t_name = (str(row["track_name"])[:45].replace(
                "<", "&lt;").replace(">", "&gt;"))
            a_name = str(
                row["artists"])[
                :40].replace(
...
st.markdown(
    '<div class="chart-card"><div class="chart-title">PCA Component Loadings</div><div class="chart-sub">Which audio features drive the most variance?</div>',
    unsafe_allow_html=True,
)

scaler_imp = StandardScaler()
X_imp = scaler_imp.fit_transform(
    df_raw.sample(
        n=min(
            20000,
            len(df_raw)),
        random_state=42)[FEATURES_FOR_CLUSTER])
...
with col_ex1:
    st.markdown(
        '<div class="chart-card"><div class="chart-title">Content Distribution</div><div class="chart-sub">Explicit vs non-explicit tracks</div>',
        unsafe_allow_html=True,
    )
    expl_counts = df_raw["explicit"].value_counts().reset_index()
    expl_counts.columns = ["explicit", "count"]
    expl_counts["label"] = expl_counts["explicit"].map(
    ...
    st.plotly_chart(
        fig_expl, use_container_width=True, config={"displayModeBar": False}
    )
    st.markdown("</div>", unsafe_allow_html=True)

with col_ex2:
    st.markdown(
        '<div class="chart-card"><div class="chart-title">Energy & Danceability: Explicit vs Clean</div><div class="chart-sub">Grouped box comparison</div>',
        unsafe_allow_html=True,
    )
    df_raw["Content"] = df_raw["explicit"].map(
        {True: "🔞 Explicit", False: "✅ Clean"})
    fig_box2 = go.Figure()
    for feat, col_b in [("energy", "#1DB954"), ("danceability", "#7c3aed")]:
        for label, line_c in [("🔞 Explicit", "#f97316"),
                              ("✅ Clean", "#60a5fa")]:
    ...
    st.plotly_chart(
        fig_box2, use_container_width=True, config={"displayModeBar": False}
    )
    st.markdown("</div>", unsafe_allow_html=True)
